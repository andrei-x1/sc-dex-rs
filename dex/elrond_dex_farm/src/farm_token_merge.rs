    ...
    + token_merge::TokenMergeModule
{
    ...
    fn get_merged_farm_token_attributes(
        ...
        replic: Option<FarmToken<Self::TypeManager>>,
    ...
    fn aggregated_reward_per_share(&self, tokens: &[FarmToken<Self::TypeManager>]) -> BigUint {
    ...
    fn aggregated_apr_multiplier(&self, tokens: &[FarmToken<Self::TypeManager>]) -> SCResult<u8> {
    ...
    fn aggregated_with_lock_rewards(
        &self,
        tokens: &[FarmToken<Self::TypeManager>],
    ) -> SCResult<bool> {
    ...
    fn aggregated_compounded_reward(&self, tokens: &[FarmToken<Self::TypeManager>]) -> BigUint {
        let mut sum = self.types().big_uint_zero();
        tokens.iter().for_each(|x| {
    ...
    fn aggregated_current_farm_amount(&self, tokens: &[FarmToken<Self::TypeManager>]) -> BigUint {
    ...
    fn aggregated_original_entering_epoch(&self, tokens: &[FarmToken<Self::TypeManager>]) -> u64 {
        let mut dataset = Vec::new();
        tokens.iter().for_each(|x| {
            dataset.push(ValueWeight {
                value: self
    ...
    fn weighted_average(&self, dataset: Vec<ValueWeight<Self::TypeManager>>) -> BigUint {
